...
했습니다. 성장 세계관(나무/별자리/행성)이 정해지면 이 아이템들을 그 위에
얹는 걸 다음 단계로 생각하고 있습니다.

보류 (PRD 참고): 성장 시각화(나무/별자리) 세계관 확정, 원본/디스플레이/썸네일 리사이즈 파이프라인(현재는 원본만 업로드), 댓글 UI, Google Calendar 연동, 가족 공유 일정(현재 일정은 개인 캘린더에만 표시).
